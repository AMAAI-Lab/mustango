...
from utils.utils import pitch_change, volume_change, volume_change_simple, speed_change, chord_pitch_change_fc, chord_speed_change_fc, beat_change_fc, bpm_change_fc, key_change_fc, format_key
from utils.prompt_functions import pitch_prompt_fc, speed_prompt_fc, cres_prompt_fc, beats_prompt_fc, chords_prompt_fc, bpm_prompt_fc, key_prompt_fc

config_path = "/666/TANGO/gh/text2music/config/augmentation_config.yaml"
#config 
...
output_path='/666/TANGO/music-caps/data_aug'
data_path='/666/TANGO/music-caps/data'
ref_file='/666/TANGO/music-caps/train_musiccaps_ep.json'
#read chords and beats from ep.json
#but read old prompts from orig.json
out_json_path='/666/TANGO/music-caps/train_musiccaps_aug.json'
    ...
    with open(out_json_path, 'w', encoding='utf-8') as out_json_file:
        ...
        for row in ref:
            ...
            for j in range(np.sum(number_of_runs)):
                ...
                music_prompts = chords_prompt_fc(chords) + ' ' + beats_prompt_fc(beats) + ' ' + bpm_prompt_fc(bpm) + ' ' + key_prompt_fc(format_key([key,altkey]))
